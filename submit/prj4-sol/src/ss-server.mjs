...
function setupRoutes(app) {
  ...
  //must be last
  app.get(`/`,open_ss(app));
  app.post(`/`,updatepg(app));
  app.get(`/ss/:ssName`, updatepgget(app));
  app.post(`/ss/:ssName`, updatepgpost(app));
...
// GET - 1
function open_ss(app) {
  return  async function(req, res) {

//console.log("base is : ",app.locals.base);
//console.log("req body is: ",req.body);
  const model =  { base: app.locals.base, fields: FIELDS_pg1 };
  res.send(app.locals.mustache.render('openss', model));
...
// POST - 1
function updatepg(app) {
  return async function(req, res) {
 
 const ssName = req.body.ssName;
 console.log("ssname is: ", ssName)
 ...
 // const model = { base: app.locals.base, fields: FIELDS_pg2 };
 // res.send(app.locals.mustache.render('updatepg', model));
}
}


// GET - 2
function updatepgget(app){
  return async function(req, res){
   const ssName = req.params.ssName;
   console.log(ssName)
    const data =  await app.locals.store.readFormulas(ssName);
   data[0].charAt();


   const model = { fields: FIELDS_pg2, ssName: ssName, data: data };
   console.log(model[ssName]);
    res.send(app.locals.mustache.render('updatepg', model));
...
// POST - 2
function updatepgpost(app){
 return async function(req, res){
  const ssName = req.body.ssName;
  const model = {base: app.locals.base, fields: FIELDS_pg2, ssName: ssName};
...
const FIELDS_pg1 = Object.keys(FIELD_INFOS_pg1).map((n) => Object.assign({name: n}, FIELD_INFOS_pg1[n]));

const FIELDS_pg2 = Object.keys(FIELD_INFOS_pg2).map((n) => Object.assign({name: n}, FIELD_INFOS_pg2[n]));
